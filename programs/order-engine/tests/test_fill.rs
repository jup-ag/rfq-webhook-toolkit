...
use litesvm_token::{get_spl_account, CreateAssociatedTokenAccount, MintTo};
...
fn prepare_test(test_mode: TestMode) -> TestEnvironment {
    ...
    svm.airdrop(&payer.pubkey(), 10 * TEST_AIRDROP).unwrap();
...
fn create_native_mint(svm: &mut LiteSVM) {
...
fn create_ata(svm: &mut LiteSVM, payer: &Keypair, owner: &Pubkey, token: &TestToken) {
    CreateAssociatedTokenAccount::new(svm, payer, &token.mint)
        .owner(owner)
...
fn mint_to(
    ...
    MintTo::new(svm, payer, &token.mint, token_account, amount)
        .token_program_id(&token.program_id)
        .send()
        .unwrap();
...
fn token_account_amount(svm: &LiteSVM, account: &Pubkey) -> u64 {
    let token_account = svm.get_account(account).unwrap();
    if token_account.owner == spl_token_2022::ID {
        get_spl_account::<spl_token_2022::state::Account>(svm, account)
            .unwrap()
            .amount
    } else {
        get_spl_account::<spl_token::state::Account>(svm, account)
            .unwrap()
            .amount
    }
}
